...
class Barang_model {
    ...
    public function getAllBarangPage($batasHalaman, $halaman_awal)
    {
        $this->db->query('SELECT * FROM ' . $this->tabel . ' INNER JOIN rak ON ' . $this->tabel . '.id_rak = rak.id_rak LIMIT ' . $halaman_awal . ', ' . $batasHalaman);
        return $this->db->resultSet();
    }
