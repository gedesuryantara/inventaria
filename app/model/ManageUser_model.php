...
class ManageUser_model {
    ...
    public function searchUser($getusername, $awalData, $limitData) {
        $keyword = '%' . $getusername . '%';
        
        // Menggunakan OFFSET dan FETCH untuk mengatur LIMIT
        $query = "SELECT * FROM `user` WHERE `username` LIKE :keyword LIMIT :limit OFFSET :offset";
        
        $this->stmt = $this->dsn->prepare($query);
        $this->stmt->bindParam(':keyword', $keyword, PDO::PARAM_STR);
        $this->stmt->bindParam(':limit', $limitData, PDO::PARAM_INT);
        $this->stmt->bindParam(':offset', $awalData, PDO::PARAM_INT);
    ...
    public function countSearchUsername($getusername){
        $keyword = '%' . $getusername . '%'; // Menambahkan wildcard (%) untuk pencarian yang lebih fleksibel
        $this->stmt = $this->dsn->prepare("SELECT * FROM `user` WHERE `username` LIKE ?");
        $this->stmt->execute([$keyword]);
        return $this->stmt->rowCount();
    }
